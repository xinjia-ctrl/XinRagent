...
async def handle_mcp_json_rpc(
    ...
    if isinstance(payload, list):
        responses = [await _handle_single(request, registry) for request in payload]
        responses = [response for response in responses if response is not None]
        return responses or Response(status_code=204)
    response = await _handle_single(payload, registry)
...
async def _handle_single(payload: dict[str, Any], registry: MCPToolRegistry) -> dict[str, Any] | None:
    ...
    if request_id is None:
        try:
            await _dispatch(method, payload.get("params") or {}, registry)
        except Exception:
            return None
        return None
    try:
        result = await _dispatch(method, payload.get("params") or {}, registry)
...
async def _dispatch(method: str, params: Any, registry: MCPToolRegistry) -> dict[str, Any]:
    ...
    if method == "tools/list":
        return {"tools": [_tool_payload(tool) for tool in registry.list_tools()]}
